...
import { IconArrowRight, IconFlame, IconGauge, IconLeaf, IconPieChart } from '@/components/icons'
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: dict.imcCalculator.meta.title,
    description: dict.imcCalculator.meta.description,
    alternates: {
      canonical: `${SITE_URL}/${lang}/calculadora-imc`,
      languages: {
        'pt-BR': `${SITE_URL}/pt/calculadora-imc`,
      },
    },
    openGraph: {
      type: 'website',
      ...
      title: dict.imcCalculator.meta.title,
      description: dict.imcCalculator.meta.description,
      url: `${SITE_URL}/${lang}/calculadora-imc`,
    },
    twitter: {
...
export default async function CalculadoraImcPage({ params }: PageProps) {
  const lang = (await params).lang
  const dict = await getDictionary(lang)
    ...
    <>
      ...
      <div className={styles.hero}>
        <div className={`container ${styles.heroInner}`}>
          ...
          <ImcCalculator dict={{ form: t.form, result: t.result }} />
        </div>
      </div>
      ...
      <article className={styles.article}>
        <div className={`container container--narrow`}>
          ...
          <section className={styles.seoBlock} aria-labelledby="sec-table">
            <h2 id="sec-table" className={styles.seoTitle}>
              {t.seo.tableTitle}
            </h2>
            {t.seo.tableText.split('\n\n').map((p: string, i: number) => (
              <p key={i} className={p.startsWith('•') ? styles.formulaBlock : undefined}>
                {p}
              </p>
            ))}
          ...
          <section className={styles.seoBlock} aria-labelledby="sec-limitations">
            <h2 id="sec-limitations" className={styles.seoTitle}>
              {t.seo.limitationsTitle}
            </h2>
            {t.seo.limitationsText.split('\n\n').map((p: string, i: number) => (
              <p key={i}>{p}</p>
            ))}
          </section>
          ...
          <section className={styles.crossLinksSection} aria-labelledby="sec-cross-links">
            ...
            <div className={styles.crossLinksGrid}>
              ...
              <Link
                href={`/${lang}/calculadora-macros`}
                className={styles.crossLinkCard}
              >
                <span className={styles.crossLinkIcon} aria-hidden="true"><IconPieChart size={20} /></span>
                <div>
                  <strong className={styles.crossLinkName}>{t.crossLinks.macros.name}</strong>
                  <p className={styles.crossLinkDesc}>{t.crossLinks.macros.desc}</p>
                </div>
                <IconArrowRight size={18} className={styles.crossLinkArrow} />
              </Link>
            </div>
